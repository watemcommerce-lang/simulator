    ...
    <div className="space-y-6 py-2">
      ...
      {data && (
        <p className="text-sm" style={{ color: "#64748B" }}>
          {filtered.length} questão(ões){topico !== "Todos" ? ` em "${topico}"` : ` — página ${page} de ${data.pagination.totalPages}`}
        </p>
      )}
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16">
          <p className="text-sm" style={{ color: "#64748B" }}>Nenhuma questão encontrada para este filtro.</p>
        </div>
      ) : (
          ...
          {filtered.map((q) => {
            ...
            return (
              <div key={q.id} className="rounded-xl overflow-hidden" style={{ border: "1.5px solid #E2D9EE", background: "#fff" }}>
                <button className="w-full flex items-center gap-3 px-4 py-3.5 text-left" onClick={() => setOpenId(isOpen ? null : q.id)}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs" style={{ color: "#94A3B8" }}>ENEM {q.ano ?? ""} · {q.nivel_dificuldade}</p>
                  </div>
                  {isOpen ? <ChevronUp className="h-4 w-4 flex-shrink-0" style={{ color: "#94A3B8" }} /> : <ChevronDown className="h-4 w-4 flex-shrink-0" style={{ color: "#94A3B8" }} />}
                </button>

                {isOpen && (
                  <div className="px-4 pb-5 space-y-4" style={{ borderTop: "1px solid #E2D9EE" }}>
                    <div className="pt-4"><LatexRenderer fontSize="sm">{q.enunciado}</LatexRenderer></div>
                    {q.url_imagem && <img src={q.url_imagem} alt="" className="max-w-full rounded-lg" />}
                    ...
                    <div className="flex gap-1.5 flex-wrap pt-1">
                      {(q.tags as string[])?.map((tag) => (
                        <span key={tag} className="text-xs px-2 py-0.5 rounded-full" style={{ background: "#E0F7F4", color: "#00897B" }}>{tag}</span>
                      ))}
                    </div>
                  </div>
                )}
