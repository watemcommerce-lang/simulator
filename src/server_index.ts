...
// =============================================================================
// Rota: importar questões do ENEM (com suporte a imagens nas alternativas)
...
app.get("/admin/import", async (req, res) => {
  ...
  try {
    ...
    for (const q of allQuestions) {
      // Alternativas — guarda texto E imagem quando existir
      const alternativas: Record<string, { text: string; file: string | null }> = {};
      for (const alt of (q.alternatives ?? [])) {
        alternativas[alt.letter] = {
          text: alt.text ?? "",
          file: alt.file ?? null,
        };
      }
      ...
      try {
        await db.insert(questions).values({
          ...
          alternativas,  // agora guarda {text, file} por alternativa
